...
export default function ActivityPage() {
  ...
  const focusSymbol = String(snapshot?.state?.focus_symbol ?? configuredSymbols[0] ?? "");
  ...
  return <div className="page-wrap">
    ...
    {tab === "trades" ? <>
      ...
      <article className="card trade-log-card">
        ...
        {trades?.items.length ? <div className="trade-table-wrap"><table className="trade-table"><thead><tr><th>Closed</th><th>Symbol</th><th>Side</th><th>Entry → Exit</th><th>Quantity</th><th>Fees</th><th>Net PnL</th><th>Source</th><th>Trigger</th></tr></thead><tbody>{trades.items.map((item) => { const confirmed = Boolean(Number(item.pnl_confirmed ?? 0)); const source = String(item.pnl_source ?? "engine"); return <tr key={String(item.id)}><td>{formatTime(item.closed_at)}</td><td><strong>{String(item.symbol)}</strong></td><td>{String(item.side ?? "—")}</td><td>{formatNumber(item.entry_price)} → {formatNumber(item.exit_price)}</td><td>{formatNumber(item.amount, 6)}</td><td>{formatNumber(item.total_fees)}</td><td className={Number(item.net_pnl) >= 0 ? "positive" : "negative"}>{formatNumber(item.net_pnl)}</td><td><span className={confirmed && source.includes("positions_history") ? "source-badge verified" : "source-badge"}>{confirmed && source.includes("positions_history") ? "Exchange verified" : source.replaceAll("_", " ")}</span></td><td>{String(item.trigger ?? "—")}</td></tr>; })}</tbody></table></div> : <Empty title="No closed trades yet" copy="Closed simulation and Live positions will appear here across all symbols." />}
      </article>
