...
export default function ActivityPage() {
  ...
  const { data: activity } = useSWR<ActivityData>(symbol ? `/api/v1/runtime/activity?symbol=${encodeURIComponent(symbol)}&limit=50` : null, api, { refreshInterval: 15000 });
  const { data: trades } = useSWR<TradeLog>(symbol ? `/api/v1/runtime/trades?symbol=${encodeURIComponent(symbol)}&outcome=${outcome}&limit=50` : null, api, { refreshInterval: 15000 });
  const events = [...(activity?.events ?? [])].reverse();
  return <div className="page-wrap">
    <PageHeading eyebrow="Audit trail" title="Activity & trades" />
    <div className="pair-view" id="workspace-pair-view" key={symbol || "activity"}>
    <div className="activity-tabs" role="tablist" aria-label="Activity view"><button type="button" role="tab" aria-selected={tab === "trades"} aria-controls="activity-panel" className={tab === "trades" ? "selected" : ""} onClick={() => setTab("trades")}>Trade log</button><button type="button" role="tab" aria-selected={tab === "events"} aria-controls="activity-panel" className={tab === "events" ? "selected" : ""} onClick={() => setTab("events")}>Engine events</button></div>
    <div id="activity-panel" role="tabpanel">
    {tab === "trades" ? <>
      ...
      <article className="card trade-log-card">
        <div className="section-heading"><div><span>{symbol || "Selected pair"}</span><h2>Closed positions</h2></div><select aria-label="Filter by outcome" value={outcome} onChange={(event) => setOutcome(event.target.value)}><option value="all">All outcomes</option><option value="win">Wins</option><option value="loss">Losses</option><option value="breakeven">Breakeven</option></select></div>
        {trades?.items.length ? <div className="trade-table-wrap"><table className="trade-table"><thead><tr><th>Closed</th><th>Symbol</th><th>Side</th><th>Entry → Exit</th><th>Quantity</th><th>Fees</th><th>Net PnL</th><th>Source</th><th>Trigger</th></tr></thead><tbody>{trades.items.map((item) => { const confirmed = Boolean(Number(item.pnl_confirmed ?? 0)); const source = String(item.pnl_source ?? "engine"); return <tr key={String(item.id)}><td data-label="Closed">{formatTime(item.closed_at)}</td><td data-label="Symbol"><strong>{String(item.symbol)}</strong></td><td data-label="Side">{String(item.side ?? "—")}</td><td data-label="Entry → Exit">{formatNumber(item.entry_price)} → {formatNumber(item.exit_price)}</td><td data-label="Quantity">{formatNumber(item.amount, 6)}</td><td data-label="Fees">{formatNumber(item.total_fees)}</td><td data-label="Net PnL" className={Number(item.net_pnl) >= 0 ? "positive" : "negative"}>{formatNumber(item.net_pnl)}</td><td data-label="Source"><span className={confirmed && source.includes("positions_history") ? "source-badge verified" : "source-badge"}>{confirmed && source.includes("positions_history") ? "Exchange verified" : source.replaceAll("_", " ")}</span></td><td data-label="Trigger">{String(item.trigger ?? "—")}</td></tr>; })}</tbody></table></div> : <Empty title={`No ${symbol || "pair"} trades yet`} copy="Closed simulation and Live positions for this pair will appear here." />}
      </article>
