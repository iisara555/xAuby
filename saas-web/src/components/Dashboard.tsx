...
type Catalog = { presets: Array<{ id: string; label: string; symbol: string; live_certified: boolean }>; risk: Record<string, { default: number }> }
...
export function Dashboard({ me, onLogout }: { me: Me; onLogout: () => void }): ReactNode {
  ...
  async function refresh(): Promise<void> {
    if (!me.tenant) return
    try { setBot(await api<Record<string, unknown>>("/api/v1/bot")); setCatalog(await api<Catalog>("/api/v1/catalog")) } catch (caught) { setError(caught instanceof Error ? caught.message : "โหลดข้อมูลไม่สำเร็จ") }
  }
  useEffect(() => {
    if (!me.tenant) return
    let active = true
    Promise.all([
      api<Record<string, unknown>>("/api/v1/bot"), api<Catalog>("/api/v1/catalog"),
  ...
  const state = (bot?.state ?? {}) as Record<string, unknown>
...
function Overview({ state, bot, onAction }: { state: Record<string, unknown>; bot: Record<string, unknown> | null; onAction: (path: string) => Promise<void> }): ReactNode {
  const connection = bot?.exchange_connection as Record<string, unknown> | null
  return <><section className="metric-grid"><Card><span>Engine</span><strong>{String(bot?.service_status ?? "unknown")}</strong><Badge tone={bot?.service_status === "active" ? "good" : "warn"}>service</Badge></Card><Card><span>Exchange</span><strong>{String(connection?.exchange_id ?? "ยังไม่เชื่อม")}</strong><small>{String(connection?.status ?? "not configured")}</small></Card><Card><span>Position</span><strong>{String(state.position_side ?? "FLAT")}</strong><small>Qty {String(state.quantity ?? "0")}</small></Card><Card><span>Mode</span><strong>{String(state.mode ?? "SIM")}</strong><small>guarded execution</small></Card></section><Card><div className="card-head"><div><h2>Engine control</h2><p>ทุกคำสั่งถูกบันทึกใน audit trail</p></div><div className="button-row"><Button onClick={() => void onAction("/api/v1/bot/start")}>Start</Button><Button className="secondary" onClick={() => void onAction("/api/v1/bot/stop")}>Stop</Button></div></div></Card></>
}
...
function Setup({ catalog, onDone, onError }: { catalog: Catalog; onDone: (text: string) => void; onError: (text: string) => void }): ReactNode {
  ...
  async function exchange(event: FormEvent<HTMLFormElement>): Promise<void> { event.preventDefault(); const data = new FormData(event.currentTarget); try { await api("/api/v1/exchange/connect", { method: "POST", body: JSON.stringify({ exchange_id: "okx", market_type: "swap", api_key: data.get("key"), api_secret: data.get("secret"), passphrase: data.get("passphrase"), withdraw_disabled_attested: data.get("attest") === "on" }) }); onDone("บันทึก Exchange แล้ว กรุณากด Test") } catch (e) { onError(e instanceof Error ? e.message : "บันทึกไม่สำเร็จ") } }
