...
export type BotSummary = {
  tenant: { id: string; slug: string; status: string; live_status: string } & Record<string, unknown>
  service_status: string
  state: { position_side?: string; quantity?: number | string; mode?: string } & Record<string, unknown>
  exchange_connection: null | ({ target_id?: string; exchange_id?: string; status?: string } & Record<string, unknown>)
}
