...
**ข้อเสนอที่ข้อมูลนี้ชี้ไป: สอง config เป็นคู่เสริมกัน ไม่ใช่คู่แข่ง** →
เป็นเคสของ **regime router** ที่มีอยู่ในโค้ดเบสแต่ **ปิดอยู่** สำหรับ XAU
(route ขาขึ้นชัดเจนไป long-only+D1, route breakdown/chop ไป long+short)
ยังไม่มี certificate ฉบับไหนประเมินทางนี้ — เป็นข้อเสนอ ไม่ใช่ผลลัพธ์
...
| **E. เปิด regime router ให้ XAU** (ใหม่) | ใช้จุดแข็งของทั้งสอง config — แต่ต้อง validate เองก่อน และ live path ถูกกั้นด้วย `regime_router_live_confirmed` ต่อคู่ |
