...
| **F. เปิด `use_d1_regime_filter` ให้ XAU (คงฝั่ง short)** ⭐ **แนะนำ** | **ชนะ config ที่ deploy อยู่ 7 ใน 8 ตัวชี้วัด** — PF 1.17→**1.51**, MDD 17.66→**12.20%**, Sharpe 0.51→**1.07**, เทรด 296→171, เดือนแย่สุด -12.01→**-6.18%** และยังเก็บ alpha ขาลงไว้ (+13.73% เทียบ cert +1.24%, ซื้อถือ -23.21%) แก้ด้วย **คีย์เดียวใน `coin_whitelist.json`** ไม่ต้องแก้โค้ด |
