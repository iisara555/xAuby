...
**ที่ยังต้องตัดสิน** — ข้อ 2 บอกว่า config ที่ deploy มี PF 1.28 / MDD 29.2% และ
MDD เกินรั้ว drawdown_guard 25% ทางเลือก:

| ทางเลือก | ผลกระทบ |
|---|---|
| **A. เปลี่ยน live เป็น long-only + D1 ON** | ไปอยู่บน config ที่ 2 งานวิจัยยืนยันตรงกัน (PF 2.00, MDD 7.6%) แต่เสีย short — ต้องรอ position ว่าง + restart แบบควบคุม |
| **B. คง config เดิม แล้วรับรองมันให้จบ** | รันโปรโตคอลเต็ม (WFA + bootstrap) บน config ที่ deploy จริง แล้วยอมรับ/ปฏิเสธด้วยตัวเลขของมันเอง — ระหว่างนั้นยังถือ exposure ที่ไม่มีหลักฐาน |
| **C. ลด XAU เป็น sim ชั่วคราว** | หยุดความเสี่ยงทันที ทำวิจัยให้จบก่อนกลับขึ้น live |
| **D. คงไว้ + เขียนรับทราบความเสี่ยงเป็นลายลักษณ์อักษร** | เร็วที่สุด แต่ MDD 29.2% ยังเกินรั้ว 25% อยู่ |
