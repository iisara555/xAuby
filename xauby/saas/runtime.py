        ...
        *,
        ...
        if self.uses_legacy(slug):
            try:
                payload = self._legacy_get(
                    "/api/trades",
                    params={"symbol": clean_symbol or "", "limit": clean_limit},
                )
                items = list(payload.get("trades") or payload.get("items") or [])
                items = self._filter_trade_outcome(items, clean_outcome)
                return self._trade_response(items[:clean_limit], source="legacy_webui", read_only=True)
            except (OSError, ValueError, RuntimeError, requests.RequestException, json.JSONDecodeError):
                return {**self._unavailable("legacy_webui", read_only=True), "items": [], "summary": {}}
