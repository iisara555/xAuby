...
PRESET_SPECS = [
    ...
    {
        "id": "okx-btc-smc-structure-long-v1",
        "target_id": "okx-swap",
        "label": "BTC SMC Structure · Research",
        "exchange_id": "okx",
        "market_type": "swap",
        "symbol": "BTCUSDT",
        "asset": "BTC",
        "strategy": "xauby_smc_pro",
        "primary_timeframe": "4h",
        "confirm_timeframe": "",
        "allowed_sides": ["long"],
        "max_leverage": 1,
        ...
        "risk_pct": 0.02,
        "max_position_per_trade_pct": 25.0,
        # Orthogonal-alpha research candidate.  The exact profile is frozen so
        # the locked OKX run can bind its evidence to a fingerprint.  It stays
        # unavailable to live tenants even if that run passes; a separate
        # forward-shadow record and operator promotion are still required.
        "live_certified": False,
        "stop_loss_required": True,
        "execution_profile": {
            "internal_length": 5,
        ...
        "strategy_traits": [
            "Research only · not approved for live trading",
            "4H structure events: BOS / CHoCH with FVG and order-block confluence",
            "Long-only finalist; long+short is an explicitly locked comparison arm",
            "Requires native-OKX certification plus 30d / 20-trade forward shadow",
        ],
    },
    {
