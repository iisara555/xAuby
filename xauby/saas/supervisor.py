...
class TenantSupervisor:
    ...
    def queue_path(self, slug: str) -> Path:
        return self.runtime_dir(slug) / "manual_orders.db"
    ...
    def _set_config_permissions(config_dir: Path) -> None:
        for name in ("bot_config.yaml", "coin_whitelist.json"):
            try:
                ...
                pass
        try:
            os.chmod(config_dir / "secrets.env", 0o600)
        except OSError:
            pass
    ...
    def provision(self, slug: str) -> None:
        ...
        if not secret_file.exists():
            secret_file.write_text("# Managed by xAuby Control Plane\n", encoding="utf-8")
    ...
    def stop(self, slug: str) -> dict[str, Any]:
        return self._systemctl("stop", slug)
    ...
    def store_credentials(self, slug: str, exchange_id: str, api_key: str,
        ...
        if not prefix or not api_key or not api_secret:
            raise ValueError("exchange, API key and API secret are required")
        if any("\n" in value or "\r" in value for value in (api_key, api_secret, passphrase)):
            raise ValueError("exchange credentials may not contain line breaks")
        lines = [
            self._environment_line(f"{prefix}_API_KEY", api_key),
            self._environment_line(f"{prefix}_API_SECRET", api_secret),
        ]
        if passphrase:
            lines.append(self._environment_line(f"{prefix}_API_PASSPHRASE", passphrase))
        path = self.config_dir(slug) / "secrets.env"
        path.write_text("\n".join(lines) + "\n", encoding="utf-8")
        try:
            os.chmod(path, 0o600)
        except OSError:
    ...
    def probe_exchange(self, slug: str) -> dict[str, Any]:
        """Run the credential/capability probe in a one-shot child process."""
        self.provision(slug)
        env = dict(os.environ)
        from dotenv import dotenv_values

        for key, value in dotenv_values(self.config_dir(slug) / "secrets.env").items():
    ...
    def set_live_mode(self, slug: str, exchange_id: str, market_type: str) -> None:
        ...
        secret_path = self.config_dir(slug) / "secrets.env"
        secret_lines = secret_path.read_text(encoding="utf-8").splitlines()
        retained = [
            line for line in secret_lines
            if not line.startswith("LIVE_TRADING=") and not line.startswith("SIMULATE_ONLY=")
        ]
        secret_path.write_text(
            "\n".join([*retained, "LIVE_TRADING=true", "SIMULATE_ONLY=false"]) + "\n",
            encoding="utf-8",
        )
        self._set_config_permissions(self.config_dir(slug))
    ...
    def set_sim_mode(self, slug: str) -> None:
        ...
        secret_path = self.config_dir(slug) / "secrets.env"
        retained = [
            line for line in secret_path.read_text(encoding="utf-8").splitlines()
            if not line.startswith("LIVE_TRADING=") and not line.startswith("SIMULATE_ONLY=")
        ]
        secret_path.write_text(
            "\n".join([*retained, "LIVE_TRADING=false", "SIMULATE_ONLY=true"]) + "\n",
            encoding="utf-8",
        )
        self._set_config_permissions(self.config_dir(slug))
