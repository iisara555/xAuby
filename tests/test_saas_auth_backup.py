...
class SaaSAuthAndBackupTests(unittest.TestCase):
    def setUp(self):
        ...
        self.settings = SaaSSettings(
            ...
            public_base_url="http://testserver", session_secret="test_session_secret_123456789_long",
    ...
    def _signup_and_verify(self, email: str = "pilot@example.com") -> tuple[TestClient, dict]:
        response = self.client.post(
            "/auth/signup", json={"email": email, "password": "StrongPassword123"}
        )
        ...
        return user_client, login.json()

    def test_signup_waits_for_owner_then_profile_compiles_safely(self):
        user_client, session = self._signup_and_verify()
        me = user_client.get("/api/v1/me").json()
        self.assertEqual(me["account_status"], "pending_approval")
        self.assertIsNone(me["tenant"])

        owner_login = self.client.post("/auth/dev-login", params={"email": "iisara555@gmail.com"})
        approved = self.client.post(
            f"/api/v1/admin/users/{me['id']}/status",
            headers={"X-CSRF-Token": owner_login.json()["csrf_token"]}, json={"status": "active"},
        )
        ...
        slug = approved.json()["tenant"]["slug"]
    ...
    def test_password_reset_is_single_use_and_revokes_sessions(self):
        user_client, _ = self._signup_and_verify("reset@example.com")
    ...
    def test_predeploy_backup_verifies_databases_configs_and_secrets(self):
        ...
        self.assertTrue(manifest["includes_secrets"])
