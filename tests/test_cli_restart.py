...
def test_live_restart_uses_controlled_restart_script(monkeypatch):
    ...
    monkeypatch.setattr(cli.os.path, "isfile", lambda path: path == "scripts/controlled_restart_engine.sh")
    monkeypatch.setattr("subprocess.run", run)
    monkeypatch.setattr("launcher.run_textual_tui", tui)

    assert cli._do_restart(live_mode=True) == 0
    run.assert_called_once_with(["scripts/controlled_restart_engine.sh"])
...
def test_update_runs_deploy_from_main_with_restart(monkeypatch):
    ...
    monkeypatch.setattr(cli.os.path, "isfile", lambda path: path == "scripts/deploy_from_github.sh")
    monkeypatch.setattr("subprocess.run", run)

    assert cli._do_update() == 0
    run.assert_called_once_with(["scripts/deploy_from_github.sh", "--restart", "--branch=main"])
